  ...
  <div class="tool-page">
    <div class="plot-inner">
      ...
      <div class="plot-layout">
        ...
        <div class="ctrl-panel">
          ...
          <div class="ctrl-block">
            ...
            <p class="fn-hint">
              支持 <code>sin(x)</code> <code>x^2</code> <code>2*x+1</code>
              <code>sqrt(x)</code>
            </p>
            <button
              class="add-fn-btn"
              :disabled="fns.length >= 4"
              @click="addFn"
            >
              ＋ 添加函数
            </button>
          ...
          <div class="ctrl-block help-block">
            ...
            <div class="help-tags">
              <code v-for="sym in symbols" :key="sym" class="sym">{{
                sym
              }}</code>
            </div>
            <p class="help-note">
              乘方用 <code>^</code>，如 <code>x^2</code>；乘法用
              <code>*</code>；函数须加括号，如 <code>sin(x)</code>
            </p>
          </div>
        </div>
      </div>
      ...
      <article class="tool-desc">
        <h2>关于函数绘图器</h2>
        <p>
          在线数学函数可视化工具，在笛卡尔坐标系中实时绘制函数图像。支持最多同时绘制
          4
          条曲线，坐标系可自由缩放和平移，适合验证数学计算结果、辅助教学演示或探索函数行为规律。
        </p>
        <h3>支持的函数与符号</h3>
        <p>
          三角函数：<code>sin(x)</code>、<code>cos(x)</code>、<code>tan(x)</code>；反三角：<code>asin</code>、<code>acos</code>、<code>atan</code>；指数与对数：<code>exp(x)</code>（即
          eˣ）、<code>log(x)</code>（自然对数）；其他：<code>sqrt(x)</code>（平方根）、<code>abs(x)</code>（绝对值）、<code>ceil</code>、<code>floor</code>；常数：<code>pi</code>（π）、<code>e</code>；乘方用
          <code>^</code>，乘法用 <code>*</code>。
        </p>
        <h3>操作说明</h3>
        <ol>
          <li>
            在函数输入框中输入表达式，如 <code>sin(x)</code> 或
            <code>x^2 - 2*x + 1</code>，图像实时更新
          </li>
          <li>
            鼠标滚轮缩放坐标系；按住鼠标拖拽平移视图；点击「重置」还原初始视角
          </li>
          <li>
            点击「添加函数」可叠加多条曲线（最多 4 条）对比，每条曲线颜色不同
          </li>
          <li>
            移动鼠标到画布上可实时读取当前坐标值；触屏设备支持双指捏合缩放
          </li>
        </ol>
      </article>
...
const placeholders = ["sin(x)", "x^2 - 1", "cos(x)", "sqrt(abs(x))"];
...
// ── Expression parser (no eval / new Function) ───────────────
...
type ASTNode =
  | { k: "n"; v: number }
  | { k: "x" }
  | { k: "u"; a: ASTNode } // unary minus
  | { k: "b"; op: string; l: ASTNode; r: ASTNode }
  | { k: "f"; fn: (...a: number[]) => number; args: ASTNode[] };
...
function buildAST(src: string): ASTNode | null {
  ...
  function addSub(): ASTNode | null {
    ...
    while (true) {
      ...
      left = { k: "b", op: t.v, l: left, r: right };
    }
    return left;
  }

  // mulDiv → unary ((*|/ unary) | (implicit) power)*
  function mulDiv(): ASTNode | null {
    let left = unary();
    if (!left) return null;
    while (true) {
      const t = peek();
      if (t?.t === "op" && (t.v === "*" || t.v === "/" || t.v === "%")) {
        ...
        if (!right) return null;
        left = { k: "b", op: t.v, l: left, r: right };
      } else if (t && (t.t === "n" || t.t === "id" || t.t === "lp")) {
        // implicit multiplication: 2x, 2(x+1), pi x  (NOT triggered by - or +)
        const right = power();
        if (!right) return null;
        left = { k: "b", op: "*", l: left, r: right };
      } else break;
    }
    return left;
  }
  ...
  // unary → (-|+) unary | power
  function unary(): ASTNode | null {
    const t = peek();
    if (t?.t === "op" && t.v === "-") {
      next();
      const a = unary();
      if (!a) return null;
      if (a.k === "n") return { k: "n", v: -a.v };
      return { k: "u", a };
    }
    if (t?.t === "op" && t.v === "+") {
      next();
  ...
  function power(): ASTNode | null {
    ...
    if (pt?.t === "op" && pt.v === "^") {
      ...
      const exp = unary();
      if (!exp) return null;
      return { k: "b", op: "^", l: base, r: exp };
    }
    return base;
  }

  // primary → num | '(' expr ')' | id '(' args ')' | id (var/const)
  function primary(): ASTNode | null {
    const t = peek();
    if (!t) return null;
    ...
    if (t.t === "id") {
      ...
      if (peek()?.t === "lp") {
        ...
        if (peek()?.t === "rp") next();
        const fn = FUNS[name] ?? FUNS[lname];
        if (!fn) return null;
        return { k: "f", fn, args };
      }

      // variable x
      if (lname === "x") return { k: "x" };

      // named constant
      const cv = CONSTS[lname];
      if (cv !== undefined) return { k: "n", v: cv };
  ...
  try {
    const ast = addSub();
    // must consume all tokens for a valid expression
...
function evalAST(node: ASTNode, x: number): number {
  switch (node.k) {
    ...
    case "x":
      return x;
    case "u":
      return -evalAST(node.a, x);
    case "b": {
      const l = evalAST(node.l, x);
      const r = evalAST(node.r, x);
      switch (node.op) {
        case "+":
          return l + r;
        case "-":
    ...
    case "f":
      return node.fn(...node.args.map((a) => evalAST(a, x)));
  }
  return NaN;
}

// Compile cache: expression string → evaluator function (or null if invalid)
const cache = new Map<string, ((x: number) => number) | null>();
...
function fnInputClass(expr: string) {
  if (!expr.trim()) return "";
  return compile(expr) ? "fn-ok" : "fn-err";
}
...
const draw = () => {
  ...
  if (axY >= 0 && axY <= H) {
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(W - 10, axY - 5);
    ctx.lineTo(W, axY);
    ctx.lineTo(W - 10, axY + 5);
    ctx.stroke();
  }
  if (axX >= 0 && axX <= W) {
    ctx.beginPath();
...
// ── View controls ─────────────────────────────────────────────
const zoomIn = () => {
  scale.value = Math.min(scale.value * 1.4, BASE_SCALE * 200);
  draw();
};
const zoomOut = () => {
  scale.value = Math.max(scale.value / 1.4, BASE_SCALE / 100);
  draw();
};
const resetView = () => {
...
// ── Function management ───────────────────────────────────────
const addFn = () => {
  if (fns.value.length < 4)
    fns.value.push({ expr: "", color: COLORS[fns.value.length] });
};
const removeFn = (i: number) => {
  fns.value.splice(i, 1);
  draw();
};
const loadExample = (expr: string) => {
  fns.value[0].expr = expr;
  draw();
};
...
const onWheel = (e: WheelEvent) => {
  ...
  scale.value = Math.min(
    Math.max(scale.value * factor, BASE_SCALE / 100),
    BASE_SCALE * 200
  );
  draw();
};
...
const onMouseMove = (e: MouseEvent) => {
  ...
  const H = c.height / dpr;
  hoverXY.value = {
    x: ((px - W / 2) / scale.value + viewX.value).toFixed(3),
    y: (-(py - H / 2) / scale.value + viewY.value).toFixed(3),
  };
...
const onTouchMove = (e: TouchEvent) => {
  if (e.touches.length === 1) {
    ...
    scale.value = Math.min(
      Math.max(scale.value * (d / lastTouchDist), BASE_SCALE / 100),
      BASE_SCALE * 200
    );
    lastTouchDist = d;
    draw();
...
.fn-input.fn-ok:not(:focus) {
  border-color: var(--c-success, #2f9e44);
}
.fn-input.fn-err {
  border-color: var(--c-danger, #e03131);
}
.fn-input.fn-err:focus {
  box-shadow: 0 0 0 2px
    color-mix(in srgb, var(--c-danger, #e03131) 15%, transparent);
}
...
/* ── Desc / related (same as other pages) ─────────────── */
.tool-desc {
  margin-top: 2rem;
  font-size: 0.9rem;
  color: var(--c-text-2);
